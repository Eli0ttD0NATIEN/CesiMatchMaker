...
for parrain in parrain_data:
    ...
    for filleul in filleul_data:
        count_match = len(returnMatches(parrain, filleul))
        top_5_match.append((filleul[2]+' '+filleul[3], count_match))
    top_5_match.pop(0)
    top_5_match = sorted(top_5_match, key=lambda x: int(x[1]), reverse=True)
    top_5_match = top_5_match[:6]
    top_5_match.append(parrain[2] + " " + parrain[3])
